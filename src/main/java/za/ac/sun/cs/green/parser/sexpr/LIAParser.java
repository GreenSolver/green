...
public class LIAParser {
	 ...
	 *
	 * @param scanner the scanner from which the expression will be extracted.
	...
	private Expression parseIntExpr(Scanner scanner) throws Exception {
		...
		if (operator.equals("c")) {
			...
			try {
				...
				throw new Exception("An integer value for a constant was expected, but something else was found");
			} catch (NoSuchElementException e) {
			...
			if (arity < 2) {
				throw new Exception(
						"Arity for operator " + operator + " should be at least 2, got " + arity + " instead");
			}

			Operation expr = new Operation(Operation.Operator.ADD, this.parseIntExpr(scanner),
					this.parseIntExpr(scanner));

			for (int tmp = 2; tmp < arity; tmp++) {
				expr = new Operation(Operation.Operator.ADD, expr, this.parseIntExpr(scanner));
			}
			...
			Operation expr = new Operation(Operation.Operator.MUL, this.parseIntExpr(scanner),
					this.parseIntExpr(scanner));

			for (int tmp = 2; tmp < arity; tmp++) {
				expr = new Operation(Operation.Operator.MUL, expr, this.parseIntExpr(scanner));
			}
			...
			Operation expr = new Operation(Operation.Operator.SUB, this.parseIntExpr(scanner),
					this.parseIntExpr(scanner));

			for (int tmp = 2; tmp < arity; tmp++) {
				expr = new Operation(Operation.Operator.SUB, expr, this.parseIntExpr(scanner));
			}
	...
	private Operation parseBoolExpr(Scanner scanner) throws Exception {
		...
		for (int i = 0; i < nrClauses; ++i) {
			...
			try {
				...
				throw new Exception(
						"Expected an integer value as the number of inequalities of a clause, but found something else");
			} catch (NoSuchElementException e) {
	 ...
	 *
	 * @param s a string in SEXPR format.
