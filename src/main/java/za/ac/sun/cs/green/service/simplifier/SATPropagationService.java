...
//                System.out.println();
                if (vars.isEmpty()) {
                    ...
                    propagations += repVisitor.getPropagations(); // TODO: is this correct?
                }
            }
            HashMap<Expression, Integer> conjuncts = new HashMap<>(); // remember conjuncts, to prevent duplicated conjuncts in answer
    ...
    private static class Tuple {
        ...
        public Tuple(IntVariable v, Expression c) {
            var = v;
            value = c;
        }
    ...
    private static class ReplaceVisitor extends Visitor {
        ...
        public ReplaceVisitor(List<Tuple> vars) {
            //eqConstraints = vars;
            map = new HashMap<>();
            for (Object t : vars) {
        ...
        private Expression applyOperation(Operation.Operator op, IntConstant l, IntConstant r) {
            simplifications++;
            switch(op) {
                case ADD: return new IntConstant(l.getValue() + r.getValue());
                case SUB: return new IntConstant(l.getValue() - r.getValue());
                case DIV: return new IntConstant(l.getValue() / r.getValue());
                case MUL: return new IntConstant(l.getValue() * r.getValue());
                case LE: return (l.getValue() <= r.getValue()) ? Operation.TRUE : Operation.FALSE;
                case LT: return (l.getValue() <  r.getValue()) ? Operation.TRUE : Operation.FALSE;
                case GE: return (l.getValue() >= r.getValue()) ? Operation.TRUE : Operation.FALSE;
                case GT: return (l.getValue() >  r.getValue()) ? Operation.TRUE : Operation.FALSE;
                default: return new Operation(op, l, r);
            }
        }

        private Expression applyOperation(Operation.Operator op, IntVariable l, IntVariable r) {
            switch(op) {
                case EQ: return (l.equals(r)) ? Operation.TRUE : new Operation(op, l, r);
                default: return new Operation(op, l, r);
            }
        }
